...
var sitePath = window.location.origin.indexOf('gigil') > 0 ? '/current/xjazz/' : (window.location.origin.indexOf('localhost') > 0 ? '/xjazz/' : '/');

var xjazz_gmaps_api_key = 'AIzaSyBaVrLRdLUMvRKLLsXPuaE4slFJS28TEEs';
var mapmarkerpath = sitePath + 'mapicon.png';
...
$(document).ready(function() {
  if ($('#google_map').length) {
    ...
    var api_path = category_id > 0 ? sitePath + 'wp-json/wp/v2/xjazz_venues/?filter[cat]='+category_id : sitePath + 'wp-json/wp/v2/xjazz_venues/' + venue_id;
    var markers = [];
    ...
    $.getJSON(api_path, function(data) {
      if (data.length) {
        $.each(data, function(index, node) {
          ...
          markers[index].contentString = '<div id="content">'+
                '<div id="siteNotice">'+
                '</div>'+
          ...
          markers[index].infowindow = new google.maps.InfoWindow({
            content: markers[index].contentString,
            maxWidth: 300
          });
          markers[index].addListener('click', function() {
            if (previousMarker !== false) {
              markers[previousMarker].infowindow.close();
            }
